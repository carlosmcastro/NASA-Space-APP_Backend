...
def quest(*ask):
	...
	if 'whatexo' in ask: #What is an exoplanet? Definition of parsecs, nearest and farthest exoplanet
		...
		near=list(data[data.st_dist==data.st_dist.min()].pl_name)
		farth=list(data[data.st_dist==data.st_dist.max()].pl_name)

		if len(near)<2:
			datn='The nearest exoplanet found is called '+near[0]+" and it's aprox. "+str(data.st_dist.min())+' parsecs away.'
		else:
			datn='The nearest exoplanets found are called '+", ".join(near)+" and they're aprox. "+str(data.st_dist.min())+' parsecs away.'
		if len(farth)<2:
			datf='The farthest exoplanet found is called '+farth[0]+" and it's aprox. "+str(data.st_dist.max())+' parsecs away.'
		else:
			datf='The farthest exoplanets found are called '+", ".join(farth)+" and they're aprox. "+str(data.st_dist.max())+' parsecs away.'
	...
	if 'teff' in ask: #Effective Temperature of star
		...
		cold=list(set(data[data.st_teff==data[data.st_teff.isnull()==False].st_teff.min()].pl_hostname))
		hot=list(set(data[data.st_teff==data[data.st_teff.isnull()==False].st_teff.max()].pl_hostname))
		
		if len(cold)<2:
			datc='The star with the lowest Effective Temperature found is called '+cold[0]+" and it's aprox. "+str(data[data.st_teff.isnull()==False].st_teff.min())+' Kelvins.'
		else:
			datc='The stars with the lowest Effective Temperature found are called '+", ".join(cold)+" and they're aprox. "+str(data[data.st_teff.isnull()==False].st_teff.min())+' Kelvins.'
		if len(hot)<2:
			dath='The star with the highest Effective Temperature found is called '+hot[0]+" and it's aprox. "+str(data[data.st_teff.isnull()==False].st_teff.max())+' Kelvins.'
		else:
			dath='The stars with the highest Effective Temperature found are called '+", ".join(hot)+" and they're aprox. "+str(data[data.st_teff.isnull()==False].st_teff.max())+' Kelvins.'
	...
	if 'teeqt' in ask: #Equilibrium Temperature of a planet. Brief description of the structure of the Earth.
		...
		cold=list(set(data[data.pl_eqt==data[data.pl_eqt.isnull()==False].pl_eqt.min()].pl_name))
		hot=list(set(data[data.pl_eqt==data[data.pl_eqt.isnull()==False].pl_eqt.max()].pl_name))
		
		if len(cold)<2:
			datc='The planet with the lowest Equilibrium Temperature found is called '+cold[0]+" and it's aprox. "+str(data[data.pl_eqt.isnull()==False].pl_eqt.min())+' Kelvins.'
		else:
			datc='The planets with the lowest Equilibrium Temperature found are called '+", ".join(cold)+" and they're aprox. "+str(data[data.pl_eqt.isnull()==False].pl_eqt.min())+' Kelvins.'
		if len(hot)<2:
			dath='The planet with the highest Equilibrium Temperature found is called '+hot[0]+" and it's aprox. "+str(data[data.pl_eqt.isnull()==False].pl_eqt.max())+' Kelvins.'
		else:
			dath='The planets with the highest Equilibrium Temperature found are called '+", ".join(hot)+" and they're aprox. "+str(data[data.pl_eqt.isnull()==False].pl_eqt.max())+' Kelvins.'
